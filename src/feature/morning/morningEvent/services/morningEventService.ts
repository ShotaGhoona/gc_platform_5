...
export async function fetchMorningEventListByRange(
  range: "last_month" | "this_month" | "next_month" | "all",
  userId?: string
): Promise<MorningEventListItem[]> {
  const params = new URLSearchParams({ range });
  if (userId) params.append("user_id", userId);
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/morning_events?${params.toString()}`);

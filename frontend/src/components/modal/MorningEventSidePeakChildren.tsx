    ...
    <div className="flex flex-col justify-between h-full">
      <div className="flex flex-col gap-2 h-full">
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs text-gray-500 bg-gray-200 rounded-lg px-2 py-1">
            {(() => {
              // duration計算
              try {
                const { calcDuration, durationLabel } = require("@/feature/morning/morningEvent/utils/timeUtils");
                const min = calcDuration(eventDetail.start_at, eventDetail.end_at);
          ...
          <p className="text-sm text-gray-500">{eventDetail.start_at.replace("T", " ").slice(5, 16)}</p>
        </div>
        {/* イベント詳細 */}
        <div className="flex-1 flex flex-col justify-between bg-white rounded-lg p-4 my-10">
          <div className="flex flex-col gap-5">
            ...
            <p className="text-base text-gray-700 whitespace-pre-wrap">{eventDetail.description}</p>
          </div>
          {/* 参加予定 */}

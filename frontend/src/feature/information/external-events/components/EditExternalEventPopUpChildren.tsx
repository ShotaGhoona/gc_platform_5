...
import { ExternalEvent, updateExternalEvent } from "../services/externalEventService";
...
export const EditExternalEventPopUpChildren = ({ onClose, event }: Props) => {
  const [title, setTitle] = useState(event.title);
  const [description, setDescription] = useState(event.description || "");
  const [startAt, setStartAt] = useState(event.startAt);
  const [endAt, setEndAt] = useState(event.endAt);
  const [selectedTagNames, setSelectedTagNames] = useState<string[]>(event.tags || []);
  const [customTag, setCustomTag] = useState("");
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [imageFile, setImageFile] = useState<File | null>(null);
  const [imageUrl, setImageUrl] = useState<string | undefined>(event.image);
  ...
  const handleImageUpload = async (file: File) => {
    try {
      ...
      console.log('外部イベント画像アップロード開始（編集）:', filePath);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateExternalEvent(event.id, {
        title,
        description,
        image: url,
        startAt,
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4 h-full w-full justify-between">
      <DialogTitle className="sr-only">外部イベントを編集</DialogTitle>
      <div className="flex flex-col gap-5 w-full h-full">
        <h2 className="text-lg font-bold mb-2">外部イベントを編集</h2>

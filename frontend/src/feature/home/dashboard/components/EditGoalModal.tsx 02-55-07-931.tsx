  ...
  const handleSave = async () => {
    ...
    for (let i = 0; i < localGoals.length; i++) {
      ...
      if (!lg.id && !lg.toDelete && lg.text.trim()) {
        // 新規追加時は今月の1日をセット
        const now = new Date();
        const monthly_start_date = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-01`;
    ...
    <div className='flex h-full'>
      ...
      <div className="flex-1 h-full flex flex-col justify-between px-8 py-8">
        ...
        <h2 className="text-2xl font-bold text-gray-800 mb-6 flex items-center">
          <span className="border-l-4 border-[#5F7392] mr-3 pl-2">今月の目標設定</span>
        </h2>
        {/* ビジョン */}
